#!/usr/bin/env python3
"""
Season test: top 5 leaderboard models in a 5-week competition.

  - deepseek/deepseek-chat-v3-0324        (Elo 1508, aggressive approver)
  - meta-llama/llama-3.3-70b-instruct     (Elo 1506, balanced)
  - openai/gpt-4.1-nano                   (Elo 1506, high volume)
  - qwen/qwen3-next-80b-a3b-instruct      (Elo 1504, low rates)
  - deepseek/deepseek-r1                   (Elo 1503, 1 match only)
"""
...
# ── Banner ────────────────────────────────────────────────────────────────
print("=" * 70)
print("  LOANVILLE SEASON — TOP 5 LEADERBOARD")
...
print(f"\n  Total evaluations: {WEEKS * COHORT_SIZE * len(MODELS)}")
print()
...
# Show validation results and error rates
print(f"\n  Validation: {'PASS' if record['validation']['valid'] else 'FAIL'}")
if record['validation']['errors']:
    for err in record['validation']['errors']:
        print(f"    ✗ {err}")
print(f"\n  Error rates per model:")
for r in record['results']:
    total = r['deals_won'] + r['deals_rejected'] + r['deals_errored']
    err_rate = r['deals_errored'] / total * 100 if total > 0 else 0
    print(f"    {r['model_id']}: {r['deals_errored']}/{total} errors ({err_rate:.0f}%)")
...
print(f"\n  {'Model':<30s} {'Persona':<12s} {'Score':>7} {'Won':>5} {'Rej':>5} {'Err':>5} {'Dflt':>5} {'Net P&L':>12}")
print(f"  {'─'*82}")
for score in season_scores:
    ...
    print(f"  {short:<30s} {persona:<12s} {score.final_score:>6.1f} {state.deals_won:>5} "
          f"{state.deals_rejected:>5} {state.deals_errored:>5} {defaults:>5} ${net:>+10,.0f}")
